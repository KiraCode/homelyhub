...
import { updatedUser } from "../../store/Users/user-action.js";

const EditProfile = () => {
  const { user, errors, loading } = useSelector((state) => state.user);
  const [avatarPreview, setAvatarPreview] = useState(
    user.avatar.url || "https://i.pravatar.cc/150?img=27"
  );
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      dispatch(updatedUser(updatedFields));
      navigate("/profile");
